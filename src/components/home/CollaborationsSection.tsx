...
export function CollaborationsSection() {
  ...
  const brands = [
    { name: 'Brand One', logo: 'B1' },
    { name: 'Brand Two', logo: 'B2' },
    { name: 'Brand Three', logo: 'B3' },
    { name: 'Brand Four', logo: 'B4' },
    { name: 'Brand Five', logo: 'B5' },
    { name: 'Brand Six', logo: 'B6' },
  ]
    ...
    <section ref={sectionRef} className="relative overflow-hidden bg-gray-950 py-16">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4">
        ...
        <div className="relative overflow-hidden">
          ...
          {/* Marquee Track */}
          <div
            className={`flex gap-16 transition-all duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
          >
            <div className="flex shrink-0 gap-16">
              {brands.map((brand, index) => (
                <div
                  key={`${brand.name}-${index}`}
                  className="flex h-20 w-32 items-center justify-center rounded-lg bg-gray-900/50 p-4 grayscale transition-all duration-300 hover:grayscale-0"
                >
                  <span className="text-xl font-bold uppercase tracking-wider text-gray-600 hover:text-white">
                    {brand.logo}
                  </span>
                </div>
              ))}
